...
function getAuthTokenFromStorage(): { token: string, expiresAt: Date } | undefined {
  try {
    ...
    // Check if the token hasn't expired. We consider it expired if less than 10 minutes left
    // until the expiration date. We use such a big interval to make sure it will be enough for
    // the current user usage session.
    return Date.now() + 600_000 <= json.expiresAt.getTime() ? json : undefined;
  } catch {
  }
}
